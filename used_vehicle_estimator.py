...
class UsedVehicleEstimator:
    ...
    def estimate_current_value(self, make: str, model: str, year: int, 
        ...
        """
        Enhanced estimation of current market value for used vehicles
        Improved logic for new cars (current year) with mileage >1000 miles to properly capture depreciation
        ...
        try:
            ...
            st.write(f"Debug: {year} {make} {model} {trim}, Age = {vehicle_age}, Mileage = {current_mileage:,}, MSRP = ${original_msrp:,.0f}")
            
            # Enhanced logic for current year vehicles with significant mileage
            if vehicle_age == 0 and current_mileage > 1000:
                # CURRENT YEAR VEHICLE WITH SIGNIFICANT MILEAGE - Apply new car depreciation
                st.write(f"Debug: Current year vehicle with {current_mileage:,} miles - applying new car depreciation logic")
                
                # Progressive depreciation based on mileage thresholds for new cars
                if current_mileage <= 5000:
                    # 1,001 - 5,000 miles: Light depreciation (demo/test drive level)
                    base_depreciation = 0.08 + (current_mileage - 1000) / 4000 * 0.07  # 8-15%
                    depreciation_type = "demo/test drive level"
                elif current_mileage <= 15000:
                    # 5,001 - 15,000 miles: Moderate depreciation (rental return level)
                    base_depreciation = 0.15 + (current_mileage - 5000) / 10000 * 0.10  # 15-25%
                    depreciation_type = "rental return level"
                elif current_mileage <= 25000:
                    # 15,001 - 25,000 miles: Higher depreciation (high usage new car)
                    base_depreciation = 0.25 + (current_mileage - 15000) / 10000 * 0.10  # 25-35%
                    depreciation_type = "high usage new car"
                else:
                    # >25,000 miles: Maximum new car depreciation
                    base_depreciation = 0.35 + min((current_mileage - 25000) / 50000 * 0.15, 0.15)  # 35-50% max
                    depreciation_type = "heavily used new car"
                
                st.write(f"Debug: Mileage category: {depreciation_type}, Base depreciation: {base_depreciation:.3f}")
                
                # Apply brand-specific modifiers for new car depreciation
                brand_modifiers = {
                    # Luxury brands - faster depreciation when used as new
                    'bmw': 1.2, 'mercedes-benz': 1.2, 'audi': 1.15, 'lexus': 1.0, 
                    'acura': 1.05, 'infiniti': 1.1, 'cadillac': 1.2, 'lincoln': 1.15,
                    'genesis': 1.1, 'volvo': 1.1,
                    
                    # Value brands - better retention even with mileage
                    'toyota': 0.85, 'honda': 0.85, 'hyundai': 0.90, 'kia': 0.90,
                    'mazda': 0.95, 'subaru': 0.90,
                    
                    # Standard brands - baseline depreciation
                    'ford': 1.0, 'chevrolet': 1.0, 'nissan': 1.0, 'volkswagen': 1.0,
                    
                    # Brands with higher depreciation
                    'dodge': 1.1, 'chrysler': 1.15, 'jeep': 0.95, 'ram': 0.95,
                    
                    # Electric/specialty brands
                    'tesla': 1.1, 'rivian': 1.2, 'lucid': 1.3
                }
                
                make_lower = make.lower()
                brand_multiplier = brand_modifiers.get(make_lower, 1.0)
                
                # Calculate final depreciation for new car with mileage
                final_depreciation = base_depreciation * brand_multiplier
                
                # Apply reasonable caps for new car depreciation
                final_depreciation = min(final_depreciation, 0.55)  # Max 55% for heavily used new cars
                final_depreciation = max(final_depreciation, 0.05)  # Min 5% depreciation
                
                estimated_value = original_msrp * (1 - final_depreciation)
                
                st.write(f"Debug: Brand multiplier ({make}): {brand_multiplier:.2f}")
                st.write(f"Debug: Final depreciation: {final_depreciation:.3f} ({final_depreciation*100:.1f}%)")
                st.write(f"Debug: Estimated value: ${estimated_value:,.0f}")
                
            elif vehicle_age > 0:
                # OLDER VEHICLE - Use traditional depreciation model
                st.write(f"Debug: Older vehicle ({vehicle_age} years) - using traditional depreciation")
                ...
                st.write(f"Debug: Estimated annual mileage: {estimated_annual_mileage:,.0f}")
                
                # Use the enhanced depreciation model for older vehicles
                try:
                    ...
                    if depreciation_schedule and len(depreciation_schedule) > 0:
                        estimated_value = depreciation_schedule[-1]['vehicle_value']
                        st.write(f"Debug: Used depreciation model result: ${estimated_value:,.0f}")
                    else:
                        # Fallback calculation with realistic depreciation curves
                        depreciation_curves = {
                            1: 0.17, 2: 0.27, 3: 0.35, 4: 0.42, 5: 0.49,
                            6: 0.55, 7: 0.60, 8: 0.64, 9: 0.67, 10: 0.70
                        }
                        depreciation_rate = depreciation_curves.get(vehicle_age, 0.75)
                        
                        # Apply brand adjustment
                        brand_adjustments = {
                            'toyota': 0.9, 'honda': 0.9, 'lexus': 0.85,
                            'bmw': 1.1, 'mercedes-benz': 1.1, 'audi': 1.05
                        }
                        adjustment = brand_adjustments.get(make_lower, 1.0)
                        depreciation_rate *= adjustment
                        
                        estimated_value = original_msrp * (1 - min(depreciation_rate, 0.85))
                        st.write(f"Debug: Fallback calculation: ${estimated_value:,.0f}")
                        
                except Exception as e:
                    st.write(f"Debug: Error in depreciation model, using fallback: {e}")
                    # Simple fallback based on age
                    estimated_value = original_msrp * (0.85 ** vehicle_age)
                
                # Apply additional mileage adjustment for high-mileage vehicles
                expected_mileage = vehicle_age * 12000  # 12k miles/year baseline
                if current_mileage > expected_mileage * 1.3:  # 30% higher than expected
                    # High mileage penalty
                    excess_ratio = (current_mileage - expected_mileage) / expected_mileage
                    mileage_penalty = min(0.2, excess_ratio * 0.15)  # Max 20% penalty
                    estimated_value *= (1 - mileage_penalty)
                    st.write(f"Debug: High mileage penalty applied: -{mileage_penalty:.1%}")
                elif current_mileage < expected_mileage * 0.7:  # 30% lower than expected
                    # Low mileage bonus
                    under_ratio = (expected_mileage - current_mileage) / expected_mileage
                    mileage_bonus = min(0.1, under_ratio * 0.10)  # Max 10% bonus
                    estimated_value *= (1 + mileage_bonus)
                    st.write(f"Debug: Low mileage bonus applied: +{mileage_bonus:.1%}")
            
            else:
                # CURRENT YEAR WITH LOW MILEAGE - Minimal depreciation
                st.write(f"Debug: Current year vehicle with low mileage ({current_mileage:,}) - minimal depreciation")
                estimated_value = original_msrp * 0.97  # 3% depreciation for new car with minimal miles
            
            # Ensure minimum reasonable value (10% of original MSRP for any condition)
            min_value = original_msrp * 0.10
            if estimated_value < min_value:
                st.write(f"Debug: Applied minimum value floor: ${min_value:,.0f}")
                estimated_value = min_value
            
            # Ensure maximum reasonable value (no more than original MSRP)
            if estimated_value > original_msrp:
                st.write(f"Debug: Applied maximum value cap: ${original_msrp:,.0f}")
                estimated_value = original_msrp
            
            final_value = round(estimated_value, 0)
            st.write(f"Debug: Final estimated value: ${final_value:,.0f}")
