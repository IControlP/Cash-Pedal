...
class PredictionService:
    ...
    def calculate_total_cost_of_ownership(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        """Calculate comprehensive TCO for a vehicle with used vehicle support"""
    ...
    def _calculate_purchase_tco(self, input_data: Dict[str, Any], 
        ...
        """Calculate TCO for vehicle purchase with used vehicle support"""
        
        analysis_years = input_data.get('analysis_years', 5)
        
        # FIXED: Define purchase_price variable that was missing
        purchase_price = input_data.get('price', input_data.get('trim_msrp', 0))
        
        # FIXED: Add is_used_vehicle variable that was also missing
        from datetime import datetime
        current_year = datetime.now().year
        vehicle_age_at_purchase = current_year - input_data['year']
        is_used_vehicle = vehicle_age_at_purchase > 0
        
        # Calculate depreciation schedule
        depreciation_schedule = self.depreciation_model.calculate_depreciation_schedule(
            initial_value=input_data['trim_msrp'],
            vehicle_make=input_data['make'],
            vehicle_model=input_data['model'],
            model_year=input_data['year'],
            annual_mileage=input_data['annual_mileage'],
            years=analysis_years
        )
        
        # Calculate financing if needed
        financing_schedule = []
        if input_data.get('loan_amount', 0) > 0:
            financing_schedule = self.financial_service.calculate_loan_payments(
                loan_amount=input_data['loan_amount'],
                interest_rate=input_data.get('loan_rate', input_data.get('interest_rate', 0.055)),
                loan_term_years=input_data['loan_term'],
                analysis_years=analysis_years
            )
        
        # FIXED: Calculate detailed maintenance schedule with starting mileage
        starting_mileage = input_data.get('current_mileage', 0)  # Get current mileage for used vehicles
        ...
        # Apply brand, shop, and regional adjustments to maintenance schedule
        ...
        # Calculate year-by-year costs with proper ownership years
        annual_breakdown = []
        category_totals = {
            'depreciation': 0,
            'maintenance': 0,
        ...
        # FIXED: Use current year for purchase calculations
        from datetime import datetime
        purchase_year = datetime.now().year  # 2025 - when the vehicle is purchased
        
        for year in range(1, analysis_years + 1):
            ownership_year = purchase_year + year - 1  # CORRECT: 2025, 2026, 2027, etc.
            
            # Depreciation
            if year <= len(depreciation_schedule):
                annual_depreciation = depreciation_schedule[year-1]['annual_depreciation']
                vehicle_value = depreciation_schedule[year-1]['vehicle_value']
            else:
                annual_depreciation = 0
                vehicle_value = purchase_price * 0.15  # Minimum value
            
            # Maintenance costs (use adjusted schedule)
            if year <= len(adjusted_maintenance_schedule):
                annual_maintenance = adjusted_maintenance_schedule[year-1]['total_year_cost']
                maintenance_activities = adjusted_maintenance_schedule[year-1]['services']
            else:
            ...
            if year <= len(depreciation_schedule):
                ...
                vehicle_value = input_data['trim_msrp'] * 0.1
            ...
            annual_insurance = self.insurance_calculator.calculate_annual_premium(
                ...
                coverage_type=input_data.get('coverage_type', 'full_coverage'),
                annual_mileage=input_data['annual_mileage'],
            ...
            if financing_schedule and year <= len(financing_schedule):
                annual_financing = financing_schedule[year-1]['annual_payment']
            
            # Total annual cost
            total_annual = annual_depreciation + annual_maintenance + annual_insurance + annual_fuel + annual_financing
            
            # FIXED: Store breakdown with correct ownership years - REMOVE ANY OLD INCONSISTENT CODE
            annual_breakdown.append({
                'year': year,
                'ownership_year': ownership_year,
                'vehicle_age': ownership_year - input_data['year'],
                'vehicle_model_year': input_data['year'],
                'cumulative_mileage': input_data['annual_mileage'] * year,
        ...
        # Calculate final metrics using actual purchase price
        total_cost = sum(category_totals.values())
        average_annual_cost = total_cost / analysis_years
        ...
        return {
            'summary': {
                ...
                'purchase_price': purchase_price,  # NEW: Include actual purchase price
                'original_msrp': input_data.get('trim_msrp', purchase_price),  # NEW: Include original MSRP
                'is_used_vehicle': is_used_vehicle  # NEW: Flag for used vehicle
            },
    ...
    def _adjust_maintenance_schedule(self, base_schedule: List[Dict[str, Any]], 
        ...
        # Get multipliers from maintenance calculator
        brand_multiplier = self.maintenance_calculator.brand_multipliers.get(vehicle_make, 1.0)
        shop_multiplier = self.maintenance_calculator.shop_multipliers.get(shop_type, 1.0)
        ...
        for year_data in base_schedule:
            ...
            for service in year_data['services']:
                # Apply all multipliers to each service cost
                adjusted_cost_per_service = (service['cost_per_service'] * 
                                           brand_multiplier * 
                                           shop_multiplier * 
                                           regional_multiplier)
                
                adjusted_total_cost_for_service = adjusted_cost_per_service * service['frequency']
                
                adjusted_services.append({
                    'service': service['service'],
                    'frequency': service['frequency'],
                    'cost_per_service': adjusted_cost_per_service,
                    'total_cost': adjusted_total_cost_for_service,
                    'shop_type': shop_type,
                    'interval_based': True
                })
                
                adjusted_total_cost += adjusted_total_cost_for_service
            
            # Add wear-based maintenance for older vehicles
    ...
    def _calculate_year_specific_wear_maintenance(self, vehicle_age: int, vehicle_make: str, 
        ...
        """Calculate wear-based maintenance for a specific year"""
        
        # Base wear cost increases with age
        base_wear_costs = {
            4: 200,   # Year 4: Minor repairs
            5: 350,   # Year 5: More frequent issues
            6: 500,   # Year 6: Major components
            7: 750,   # Year 7: Significant wear
            8: 1000,  # Year 8: Expensive repairs
            9: 1250,  # Year 9: High maintenance
            10: 1500  # Year 10+: Major overhauls
        }
        
        base_cost = base_wear_costs.get(min(vehicle_age, 10), 1500)
        
        # Apply multipliers
        brand_multiplier = self.maintenance_calculator.brand_multipliers.get(vehicle_make, 1.0)
        shop_multiplier = self.maintenance_calculator.shop_multipliers.get(shop_type, 1.0)
        
        return base_cost * brand_multiplier * shop_multiplier * regional_multiplier
